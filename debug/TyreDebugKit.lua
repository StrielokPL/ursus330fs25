...
-- 0.0.3.4 spring-control A/B prerelease. Remove from the next stable release.
-- Log prefix: [TYREDBG]
...
if not TyreDebugKit.installed then
    ...
    Logging.info("[TYREDBG] TyreDebugKit 0.0.3.4 installed; read-only spring-control trace; target=%s trace=%dms detail=%dms pressureScan=%dms", table.concat(CFG.targetFileSuffixes, ","), CFG.traceIntervalMs, CFG.detailIntervalMs, CFG.pressureIntervalMs)
end
